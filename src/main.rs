...
fn main() -> () {
    ...
    let stream = init_portaudio_with_defaults_f32(1i32, 44100., 64u32);
    ...
    for i in notes.iter(){
        play_sine_wave(stream, *i as f32, 0.4, 44100., 64u32);
    }
...
fn init_portaudio_with_defaults_f32(channel_count: i32, sample_rate: f64, buffer_size: u32) -> pa::PaStream<f32>{
    ...
    let err = stream.open(None, Some(&stream_params_out), sample_rate, buffer_size, types::PaClipOff);
